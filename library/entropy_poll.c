                           ...
                           size_t *olen )
{
    ...
    if (hAdvApi == NULL) hAdvApi = LoadLibraryA("advapi32.dll");
    if (hAdvApi != NULL) {
        RtlGenRandomPtr pRtlGenRandom = (RtlGenRandomPtr)GetProcAddress(hAdvApi, "SystemFunction036");
        if (pRtlGenRandom != NULL) {
            if (pRtlGenRandom(output, (ULONG)len)) {
                *olen = len;
                return 0;
            }
        }
